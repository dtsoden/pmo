...
async function enhanceCapacityData() {
  ...
  console.log(`Range: ${format(monthStart, 'MMM d')} - ${format(today, 'MMM d')}\n`);
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    for (const workDay of workDays) {
      ...
      const hoursThisDay = Math.min(
        randomInt(4, 8) + (Math.random() * 2), // 4-10 hours with decimal
        remainingHours,
        dailyHours * 1.3 // Max 30% over daily capacity
      );
